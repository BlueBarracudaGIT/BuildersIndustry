package com.builderboy.bim.world.ores;
...
public class ModOreFeature extends Feature<ModOreFeatureConfig> {
    ...
    public static final ModOreFeature INSTATNCE = new ModOreFeature(ModOreFeatureConfig::deserialize);

    public ModOreFeature(Function<Dynamic<?>, ? extends ModOreFeatureConfig> configFactory) {
        super(configFactory);
    }
        ...
        for (int l1 = k; l1 <= k + j1; ++l1) {
            for (int i2 = i1; i2 <= i1 + j1; ++i2) {
                if (l <= world.getHeight(Heightmap.Type.OCEAN_FLOOR_WG, l1, i2)) {
                    return this.func_207803_a(world, rand, config, d0, d1, d2, d3, d4, d5, k, l, i1, j1, k1);
                }
            }
        }
                                ...
                                for (int j2 = l; j2 <= k1; ++j2) {
                                    ...
                                    if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
                                        ...
                                        if (!bitset.get(k2)) {
                                            ...
                                            if (config.target.test(worldIn.getBlockState(blockpos$mutableblockpos))) {
